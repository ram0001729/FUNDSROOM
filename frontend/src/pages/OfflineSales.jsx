...
import { ShoppingCart, IndianRupee, Store, Plus, X, Truck } from 'lucide-react';
...
const OfflineSales = () => {
  ...
  const fetchOrders = async () => {
    try {
      const res = await api.get('/sales-orders?source=OFFLINE');
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Loading offline sales...</div>;

  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Offline Sales</h1>
          <p className="text-gray-500 text-sm">Manage in-store and direct walk-in orders</p>
        </div>
        <button onClick={openModal} className="bg-amber-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-amber-700 transition-colors flex items-center gap-2">
          <Plus size={18} /> Record Offline Sale
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl">
        <div className="bg-gradient-to-br from-amber-50 to-white rounded-2xl border border-amber-100 shadow-sm p-6 flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="text-sm text-amber-600 font-semibold uppercase tracking-wider">Total In-Store Orders</div>
            <div className="p-2 bg-amber-100 rounded-lg text-amber-600"><ShoppingCart size={18} /></div>
          </div>
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-white rounded-2xl border border-emerald-100 shadow-sm p-6 flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="text-sm text-emerald-600 font-semibold uppercase tracking-wider">Offline Revenue</div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
           <h2 className="font-semibold text-gray-800 flex items-center gap-2">
             <Store size={18} className="text-amber-500" /> Physical Store
           </h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
                ...
                <tr key={order.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="p-4">
                    {order.status === 'Created' && (
                      <button onClick={() => handleDynamicChallan(order.id)} className="text-xs bg-amber-50 text-amber-600 hover:bg-amber-100 px-3 py-1.5 rounded-lg font-semibold flex items-center gap-1 transition-colors">
              ...
              {orders.length === 0 && (
                <tr>
                  <td colSpan="6" className="p-8 text-center text-gray-500">No offline orders found</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white">
              <h2 className="text-xl font-bold">Record Offline Sale</h2>
            ...
            <form onSubmit={handleCreateOrder} className="p-6 space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button type="button" onClick={addItemRow} className="text-sm text-amber-600 font-bold hover:underline">+ Add Item</button>
                </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button type="submit" className="bg-amber-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-amber-700 shadow-sm">Save Offline Sale</button>
              </div>
            </form>
          </div>
        </div>
